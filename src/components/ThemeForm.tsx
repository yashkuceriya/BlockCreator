...
const COLOR_PRESETS = [
  { label: 'Ocean Blue', value: 'Deep navy primary, sky blue accent, white base, slate gray text', colors: ['#1a365d', '#3b82f6', '#ffffff', '#64748b'] },
  { label: 'Forest', value: 'Dark green primary, emerald accent, cream base, earth brown text', colors: ['#14532d', '#10b981', '#fefce8', '#78716c'] },
  { label: 'Sunset', value: 'Warm terracotta primary, coral accent, cream base, charcoal text', colors: ['#9a3412', '#f97316', '#fffbeb', '#44403c'] },
  { label: 'Midnight', value: 'Black primary, electric purple accent, dark gray base, white text', colors: ['#0f0f0f', '#8b5cf6', '#1a1a2e', '#f8fafc'] },
  { label: 'Rose Gold', value: 'Dusty rose primary, gold accent, soft pink base, dark burgundy text', colors: ['#be123c', '#d4a853', '#fff1f2', '#4c0519'] },
  { label: 'Arctic', value: 'Cool gray primary, cyan accent, white base, slate text', colors: ['#334155', '#06b6d4', '#f8fafc', '#475569'] },
];

const TYPOGRAPHY_OPTIONS = [
  { label: 'Modern Sans', value: 'Inter for headings and body — clean, geometric, tech-forward' },
  { label: 'Editorial Serif', value: 'Playfair Display headings with Source Sans body — elegant, editorial' },
  { label: 'Bold Geometric', value: 'Space Grotesk headings with Inter body — bold, modern, startup' },
  { label: 'Classic Serif', value: 'DM Serif Display headings with Literata body — timeless, refined' },
  { label: 'Playful Round', value: 'Sora headings with Nunito body — friendly, approachable, rounded' },
  { label: 'Minimal Mono', value: 'JetBrains Mono headings with Inter body — developer, technical, precise' },
];

const LAYOUT_OPTIONS = [
  { label: 'Centered Reading', value: 'Narrow content width (720px), centered, generous vertical spacing' },
  { label: 'Wide Showcase', value: 'Wide content (1200px), full-width hero, spacious sections' },
  { label: 'Magazine Grid', value: 'Multi-column layout, featured post grid, sidebar-style sections' },
  { label: 'Minimal Single', value: 'Single column, large typography, lots of whitespace, focused reading' },
...
export const ThemeForm = forwardRef<HTMLFormElement, ThemeFormProps>(function ThemeForm({ onSubmit, disabled, onLoadDemo }, ref) {
  ...
  const selectInput = 'w-full px-3.5 py-2.5 bg-[var(--color-bg-input)] border border-[var(--color-border-input)] rounded-[var(--radius-lg)] text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/15 focus:border-[var(--color-border-focus)] disabled:opacity-40 transition-all text-sm appearance-none cursor-pointer';
  const textInput = 'w-full px-3.5 py-2.5 bg-[var(--color-bg-input)] border border-[var(--color-border-input)] rounded-[var(--radius-lg)] text-[var(--color-text)] placeholder-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]/15 focus:border-[var(--color-border-focus)] disabled:opacity-40 transition-all text-sm';
          ...
          {onLoadDemo && (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-[var(--color-text-muted)]">Load &ldquo;Aurora Studio&rdquo; instantly — no API key needed</p>
              </div>
          ...
          <div className="grid grid-cols-1 gap-1.5">
            {EXAMPLES.map((ex) => (
              <button
                ...
                className="group flex items-center gap-3 px-3 py-2.5 text-left rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-[var(--color-bg-card)] hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent-soft)] hover:shadow-sm transition-all duration-200"
              >
                <div className="w-7 h-7 rounded-md bg-[var(--color-bg-muted)] flex items-center justify-center shrink-0 group-hover:bg-[var(--color-accent)]/10 transition-colors">
                  <svg className="w-3.5 h-3.5 text-[var(--color-text-muted)] group-hover:text-[var(--color-accent)] transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d={ex.icon} /></svg>
                </div>
                <div className="min-w-0 flex-1">
                ...
                <svg className="w-3.5 h-3.5 text-[var(--color-text-muted)] shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5" /></svg>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-[var(--radius-xl)] shadow-[var(--shadow-card)] overflow-hidden transition-all duration-300">
        <button type="button" onClick={() => setShowConstraints(!showConstraints)} className="flex items-center justify-between w-full p-4 hover:bg-[var(--color-bg-hover)]/50 transition-colors">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">Style Options</span>
          </div>
          <div className="flex items-center gap-2">
            {(colorPreferences || typographyPreferences || layoutPreferences) && (
              <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-[var(--color-accent-soft)] text-[var(--color-accent)] font-bold">{[colorPreferences, typographyPreferences, layoutPreferences].filter(Boolean).length} set</span>
            )}
        ...
        {showConstraints && (
          <div className="px-4 pb-4 space-y-4 animate-[fadeIn_0.15s_ease-out] border-t border-[var(--color-border)]/50">
            {/* Theme Slug */}
            <div className="pt-3">
              <label htmlFor="slug" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Theme Name</label>
              <input id="slug" type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="my-portfolio" disabled={disabled} className={textInput} />
            </div>

            {/* Color Palette — Visual presets */}
            <div>
              <label className="block text-xs font-medium text-[var(--color-text-secondary)] mb-2">Color Palette</label>
              <div className="grid grid-cols-3 gap-2 mb-2">
                {COLOR_PRESETS.map((preset, i) => (
                  <button
                    ...
                    type="button"
                    disabled={disabled}
                    onClick={() => { setColorPreferences(preset.value); setSelectedColorPreset(i); }}
                    className={`p-2 rounded-[var(--radius-lg)] border text-left transition-all duration-200 disabled:opacity-40 ${
                      selectedColorPreset === i
                        ? 'border-[var(--color-accent)] bg-[var(--color-accent-soft)] shadow-sm'
                        : 'border-[var(--color-border)] hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-bg-hover)]'
                    }`}
                  >
                    <div className="flex gap-0.5 mb-1.5">
                      {preset.colors.map((c, j) => (
                        <div key={j} className="w-4 h-4 rounded-full border border-black/10" style={{ backgroundColor: c }} />
                      ))}
                    </div>
                    <span className="text-[10px] font-medium text-[var(--color-text-secondary)]">{preset.label}</span>
                  </button>
                ))}
              </div>
              <input
                ...
                onChange={(e) => { setColorPreferences(e.target.value); setSelectedColorPreset(null); }}
                placeholder="Or describe custom colors..."
                disabled={disabled}
                className={`${textInput} text-xs`}
              />
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  ...
                  onChange={(e) => setTypographyPreferences(e.target.value)}
                  disabled={disabled}
                  className={selectInput}
                >
                  <option value="">Choose a font pairing...</option>
                  {TYPOGRAPHY_OPTIONS.map((opt) => (
                    <option key={opt.label} value={opt.value}>{opt.label} — {opt.value.split('—')[1]?.trim() || ''}</option>
                  ))}
                </select>
            ...
            {/* Layout — Dropdown */}
            <div>
              <label htmlFor="layout" className="block text-xs font-medium text-[var(--color-text-secondary)] mb-1.5">Layout Style</label>
              <div className="relative">
                <select
                  id="layout"
                  ...
                  className={selectInput}
                >
                  <option value="">Choose a layout style...</option>
